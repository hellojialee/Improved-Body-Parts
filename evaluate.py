...
def process(input_image, params, model, model_params, heat_layers, paf_layers):
    ...
    # ###############   使用 flip 技术进行结果assemble ##################################
    flip_heat_ord = np.array([0, 1, 5, 6, 7, 2, 3, 4, 11, 12, 13, 8, 9, 10, 15, 14, 17, 16, 18, 19])  # 两个背景如何处理
    flip_paf_ord = np.array([0, 2, 1, 4, 3, 6, 5, 8, 7, 12, 13, 14, 9, 10, 11, 18, 19, 20, 15, 16, 17, 21, 23, 22])
    swap_image = oriImg[:, ::-1, :]
    flip_heat, flip_paf = predict(swap_image, params, model, model_params, heat_layers, paf_layers)
    heatmap_avg = (heat + flip_heat[:, ::-1, :][:, :, flip_heat_ord]) / 2
    paf_avg = (paf + flip_paf[:, ::-1, :][:, :, flip_paf_ord]) / 2
    # #################################################################################
...
def validation(model, dump_name, validation_ids=None, dataset='val2017'):
    ...
    if validation_ids == None:   # todo: we can set the validataion image ids here  !!!!!!
        validation_ids = cocoGt.getImgIds()  # [:1001 在这里可以设置validate图片的大小
    # # #############################################################################
