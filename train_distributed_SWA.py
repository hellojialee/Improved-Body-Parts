...
parser.add_argument('--sync_bn',  action='store_true', default=False, help='enabling apex sync BN.')  # Freeze BN
...
def train(epoch):
    ...
    model.apply(set_bn_eval_fp32)  # freeze layer recursively.

    if args.distributed:
        train_sampler.set_epoch(epoch)
    # scheduler.step()  use 'adjust learning rate' instead
    adjust_learning_rate_cyclic(optimizer, epoch, 78, args.swa_freq)  # start_epoch
...
def adjust_learning_rate_cyclic(optimizer, current_epoch, start_epoch, swa_freqent=5, lr_max=2e-5, lr_min=1e-5):
